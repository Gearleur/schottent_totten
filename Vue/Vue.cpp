...
namespace Vue{
    ...
    bool handleCollinMaillard(Model::Player *pPlayer, Model::Player *adversaire, Model::Board *pBoard, int borderPosition, Model::Controller *controller) {
        int sumA = 0;
            ...
            if (sumA == sumB)    //dans le cas d'égalité totale, on retourne vrai pour le joueur appelant
                return true;
            else if (pPlayer->getId() == 0 && adversaire->getId() == 1)
                return sumA > sumB;
            else
                return sumB > sumA;
        }

    std::vector<Model::Card*> handleCombatdeboue(Model::Board *pBoard, const std::vector<Model::Card*> *borderSide, Model::Controller *controller) //modifie le border
    {
        std::vector<Model::Card*> updatedBorderSide;
        for (auto card : *borderSide) {
            if (card->getName() != "Combatdeboue") {
                updatedBorderSide.push_back(card);
            } else {
                pBoard->getDiscardDeck()->addCard(card);
            }
        }
        return updatedBorderSide;
    }
    ...
    void handleBansheePlayer(Model::Player *pPlayer, std::vector<Model::Card*> borderSide, Model::Board *pBoard, Model::Controller *controller)
    {
        ...
        do {
            ...
            std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
            std::cin >> cardChoice;
        }while(cardChoice <0 || cardChoice > borderSide.size());
        pBoard->getDiscardDeck()->putBack(borderSide[cardChoice]);
    }

    void handleBanshee(Model::Player *pPlayer, int borderPosition, Model::Board *pBoard, Model::Controller *controller)    //borderSide est le côté adverse du border
    {
        if(pPlayer->getId() == 0 && pBoard->getBorders()[borderPosition]->getOwner() == nullptr) {
            handleBansheePlayer(pPlayer, pBoard->getBorders()[borderPosition]->getRightBorder(), pBoard, controller);
            for (auto card : pBoard->getBorders()[borderPosition]->getLeftBorder()) {
                if (card->getName() == "Banshee") {
                    pBoard->getBorders()[borderPosition]->removeCardLeft(card);
                    delete card;
                }
            }
            }
        else if(pPlayer->getId() == 1 && pBoard->getBorders()[borderPosition]->getOwner() == nullptr){
            handleBansheePlayer(pPlayer, pBoard->getBorders()[borderPosition]->getLeftBorder(), pBoard, controller);
            for (auto card : pBoard->getBorders()[borderPosition]->getRightBorder()) {
                if (card->getName() == "Banshee") {
                    pBoard->getBorders()[borderPosition]->removeCardRight(card);
                    delete card;
                }
            }
        }
    ...
    void handleTraitre(Model::Player *pPlayer, int borderPosition, Model::Board *pBoard, Model::Controller *controller)
    {
    ...
    {
        if(!handlepiocheTactic(pPlayer, pBoard ))
            handlepiocheTactic(pPlayer, pBoard, 9);
        int number = 0;
        int cardChoice = 0;
        std::cout << "Quelle carte voulez vous remettre dans son deck ?" << std::endl;
        std::cin >> cardChoice;
        do
        {while(cardChoice <0 || cardChoice > pPlayer->getHand().size()) {
                int index = 1;
                int changeline = 0;
